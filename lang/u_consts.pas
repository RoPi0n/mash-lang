...
var
  ...
  Cnst2: TConstant;
begin
  if not CheckName(Cnst.c_names[0]) then
    AsmError('Invalid constant name: "' + Cnst.c_names[0] + '".');
  c := 0;
  while c < Constants.Count do
  begin
    if TConstant(Constants[c]).c_names.IndexOf(Cnst.c_names[0]) <> -1 then
      AsmError('Dublicate constant names: "' + Cnst.c_names[0] + '"');
    Inc(c);
  end;
  c := 0;
  while c < Constants.Count do
...
begin
  ...
  begin
    ...
    begin
      Constant := TConstant.Create;
      Constant.c_names.Add(Tk(Lines[c], 2));
      Constant.c_type := ctUnsigned64;
